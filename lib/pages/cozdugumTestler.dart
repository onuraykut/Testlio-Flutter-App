...
class _CozdugumTestlerState extends State<CozdugumTestler> {
  ...
  final testBulunmuyor = ['Paylaştığınız test bulunmuyor','Gizlediğiniz test bulunmuyor'];
  int whichTitle = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return appBarWithScaffold(testlerTasarim(width, height),
        GradientColors.Background1, titles[whichTitle],scaffoldKey: _scaffoldKey);
  }
              ...
              builder: (context, TestState state) {
                if (state is TestUninitialized) {
                  ...
                  if(state.rooms.length == 0){
                    return Center(
                      child: Text(testBulunmuyor[whichTitle],style: TextStyle(color: Colors.white,fontSize: 17),),
                    );
                  }
